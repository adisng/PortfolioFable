    ...
    <section id="about" className="mx-auto max-w-7xl px-4 py-24 md:py-32">
      ...
      <div className="hidden gap-4 md:grid md:grid-cols-3">
        ...
        <Reveal className="md:row-span-2">
          <article className="glass flex flex-col p-6 md:h-full">
            <p className="mb-3 font-mono text-xs tracking-[0.3em] text-muted-foreground">
              01/
            </p>
            <h2 className="text-2xl font-bold text-foreground">
              Aditya{' '}
        ...
        {/* Connect card */}
        <Reveal delay={0.2} className="md:row-span-2">
          <article className="glass flex flex-col p-6 md:h-full">
            <span className="inline-flex w-fit items-center gap-2 border-2 border-accent bg-secondary px-3 py-1.5 font-mono text-xs font-medium uppercase tracking-wider text-foreground">
              <span
            ...
            <button
              type="button"
              onClick={copyEmail}
              className="mt-8 flex items-center gap-2 break-all text-left font-serif text-lg sm:text-xl italic text-foreground transition-colors hover:text-accent"
            >
              {copied ? (
            ...
            <a
              href="#contact"
              className="brutal-shadow-accent mt-auto flex items-center justify-center gap-2 border-2 border-foreground bg-primary py-4 font-mono text-sm font-bold uppercase tracking-widest text-primary-foreground transition-transform hover:-translate-x-0.5 hover:-translate-y-0.5"
            >
              Connect Now <ArrowUpRight className="size-4" />
            </a>
          </article>
        ...
        {/* Stats card */}
        <Reveal delay={0.15}>
          <article className="glass grid h-auto grid-cols-2 gap-px bg-border p-0">
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="flex flex-col justify-center bg-card p-4"
              >
                <p className="text-2xl font-black text-foreground sm:text-4xl">
                  {stat.value}
                </p>
                <p className="mt-1 font-mono text-[10px] uppercase leading-tight tracking-wider text-muted-foreground">
                  {stat.label}
                </p>
              </div>
            ))}
