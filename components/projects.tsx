...
export function Projects() {
  ...
  return <section id="work" className="relative overflow-hidden border-y-2 border-foreground bg-background" aria-label="Case studies">
    <header className="border-t border-foreground/20 px-4 py-16 sm:px-8 md:px-16 lg:px-24">
      <p className="font-mono text-xs uppercase tracking-[0.4em] text-muted-foreground">[ Case Studies ]</p>
      <h2 className="mt-5 max-w-3xl text-balance font-serif text-4xl font-bold tracking-tight text-foreground sm:text-5xl md:text-7xl">Selected Work</h2>
      <p className="mt-5 max-w-xl leading-relaxed text-muted-foreground">A curated set of deep-dives into the products I&apos;ve built — architecture, decisions, tradeoffs, and outcomes.</p>
    </header>
    <div className="relative flex min-h-screen">
      <AnimatePresence mode="wait" custom={direction} initial={false}>
        <motion.article key={project.id} custom={direction} initial={{ x: direction * 60, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: direction * -60, opacity: 0 }} transition={{ type: 'spring', stiffness: 80, damping: 20 }} drag="x" dragConstraints={{ left: 0, right: 0 }} onDragEnd={(_, info) => { if (info.offset.x < -60) next(); if (info.offset.x > 60) previous() }} className="relative flex min-h-screen w-full shrink-0 touch-pan-y flex-col justify-center px-4 py-24 sm:px-8 md:px-16 lg:px-24">
          <CaseStudy3D type={project.three} />
          <p className="absolute left-4 top-6 z-20 font-mono text-xs uppercase tracking-[0.4em] text-muted-foreground sm:left-8">[ Case Studies ]</p><p className="absolute right-4 top-6 z-20 font-mono text-xs tracking-widest text-muted-foreground sm:right-8">{project.id} / {String(projects.length).padStart(2, '0')}</p>
          <div className="relative z-10 grid gap-12 lg:grid-cols-[40%_60%] lg:gap-20">
            <div className="flex flex-col justify-center"><h2 className="mt-5 max-w-xl text-balance text-5xl font-bold tracking-tighter text-foreground sm:text-6xl md:text-8xl">{project.name}</h2><p className="mt-5 max-w-lg font-mono text-xs uppercase tracking-widest text-accent">{project.tag.split(' · ').map((tag, i) => <span key={tag}>{i > 0 && <span className="text-foreground/30"> · </span>}{tag}</span>)}</p><dl className="mt-12 border-t border-foreground/20 font-mono text-xs uppercase tracking-widest"><div className="flex justify-between gap-4 border-b border-foreground/20 py-4"><dt className="text-muted-foreground">/ Role</dt><dd>{project.role}</dd></div><div className="flex justify-between gap-4 border-b border-foreground/20 py-4"><dt className="text-muted-foreground">/ Timeline</dt><dd>{project.timeline}</dd></div><div className="flex justify-between gap-4 border-b border-foreground/20 py-4"><dt className="text-muted-foreground">/ Type</dt><dd className="text-right">{project.tag.split(' · ')[0]}</dd></div></dl><div className="mt-8 flex flex-wrap gap-3">{project.live && <a className="brutal-hover inline-flex items-center gap-2 border border-foreground px-4 py-3 font-mono text-xs uppercase tracking-widest" href={project.live} target="_blank" rel="noreferrer">View live <ExternalLink data-icon="inline-end" /></a>}{project.github && <a className="brutal-hover border border-foreground px-4 py-3 font-mono text-xs uppercase tracking-widest" href={project.github} target="_blank" rel="noreferrer">GitHub</a>}</div><MediaPreview image={project.image} video={project.video} /></div>
            <div className="flex flex-col justify-center border-l-0 pr-0 lg:pl-8 lg:pr-12"><Block label="The problem" text={project.problem} /><Block label="Approach & methodology" text={project.approach} /><Block label="Key engineering challenges" text={project.challenges} /><Block label="Outcome" text={project.outcome} /><div className="mt-8 border-t border-foreground/10 pt-6"><p className="font-mono text-xs uppercase tracking-[0.3em] text-accent">/ Stack</p><ul className="mt-4 flex flex-wrap gap-2">{project.stack.map((item) => <li key={item} className="border border-foreground/15 px-3 py-1 font-mono text-xs uppercase tracking-widest text-muted-foreground">{item}</li>)}</ul></div></div>
          </div>
        </motion.article>
      </AnimatePresence>
...
function Block({ label, text }: { label: string; text: string }) { return <div className="mt-8 border-t border-foreground/20 pt-6"><p className="font-mono text-xs uppercase tracking-[0.3em] text-accent">/ {label}</p><p className="mt-3 max-w-2xl leading-relaxed text-muted-foreground">{text}</p></div> }
